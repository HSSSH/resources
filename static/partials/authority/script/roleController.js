define(['app'], function (app) {
    app.controller("roleCtrl", function ($scope, $http, $rootScope, $uibModal, $state, sucHelpers) {
        $scope.role = {};
        $scope.role.table = {
            isActive: true,
        ...
        $scope.role.refresh = function () {
            $http.get('rest/authox/roles').then(function (data) {
                ...
                $scope.role.table.content = data.slice(0, $scope.role.pages.pageSize);
            })
        }
        $scope.role.refresh();
        ...
        $scope.role.search = function () {
            $http.get('rest/authox/roles/like?key=' + $scope.role.table.searchName + '&value=' + ($scope.role.searchValue || '')).then(function (data) {
                $scope.role.table.content = data.data;
            })
        }
        ...
        if($scope.suc&&$scope.suc.role){
        	sucHelpers.extend($scope.role, $scope.suc.role);
        }

    }).controller('roleSelectCtrl', function ($scope, $stateParams, $http, sucHelpers, $uibModal) {
        $scope.role={};
        ...
        $scope.addNewAutho = function (mess) {
            ...
            var data = {};
            data.ops=[];
            angular.forEach($scope.newAutho.op,function(value){
            	data.ops.push(value.operaName);
            })
    ...
    /*        data.ops = angular.copy($scope.newAutho.op);*/
        ...
        $scope.updateAutho = function (autho) {
            ...
            var data = {};
            data.ops=[];
            angular.forEach(autho.op,function(value){
            	data.ops.push(value.operaName);
            })
