define(['app'], function(app) {
    app.controller('checkWaitingController',['$scope', '$http', '$rootScope','$state','selectOptionSer','dialogWindow','allOpenWindow',function($scope, $http, $rootScope,$state,selectOptionSer,dialogWindow,allOpenWindow) {
        $scope.searchItem = {
            dwmc:'',//单位名称
            htbh:'',//合同编号
            tdzh:'',//土地证号
        ...
        $scope.currentList = function () {
            $http.get("/departmentaudit/tobeaudit?pageNo="+$scope.page.pageNo+"&pageSize="+$scope.page.pageSize,{params:$scope.searchItem}).then(function (result) {
        ...
        $scope.passSingle = function (item,flag) {
            var str = '确认不通过?';
            var url = "/departmentaudit/auditnotpassone";
            if(flag){
                str = '确认通过?';
                url = "/departmentaudit/auditpassone";
        ...
        $scope.passMultiple = function (flag) {
            var str = '批量确认不通过?';
            var url = "/departmentaudit/auditnotpassmore";
            if(flag){
                str = '批量确认通过?';
                url = "/departmentaudit/auditpassmore";
                dialogWindow.confirm('提示',str,function () {
    ...
    app.controller('checkPassController',['$scope', '$http', '$rootScope','$state','selectOptionSer','dialogWindow','allOpenWindow',function($scope, $http, $rootScope,$state,selectOptionSer,dialogWindow,allOpenWindow){
        $scope.searchItem = {
            dwmc:'',//单位名称
            htbh:'',//合同编号
            tdzh:'',//土地证号
        ...
        //获取当前项目列表信息
        $scope.currentList = function () {
            $http.get("/departmentaudit/passaudit?pageNo="+$scope.page.pageNo+"&pageSize="+$scope.page.pageSize,{params:$scope.searchItem}).then(function (result) {
    ...
    app.controller('checkRejectController',['$scope', '$http', '$rootScope','$state','selectOptionSer','dialogWindow','allOpenWindow',function($scope, $http, $rootScope,$state,selectOptionSer,dialogWindow,allOpenWindow){
        $scope.searchItem = {
            dwmc:'',//单位名称
            htbh:'',//合同编号
            tdzh:'',//土地证号
        ...
        //获取当前项目列表信息
        $scope.currentList = function () {
            $http.get("/departmentaudit/notpassaudit?pageNo="+$scope.page.pageNo+"&pageSize="+$scope.page.pageSize,{params:$scope.searchItem}).then(function (result) {
    ...
    app.controller('checkDeleteController',['$scope', '$http', '$rootScope','$state','selectOptionSer','dialogWindow','allOpenWindow',function($scope, $http, $rootScope,$state,selectOptionSer,dialogWindow,allOpenWindow){
        $scope.searchItem = {
            dwmc:'',//单位名称
            htbh:'',//合同编号
            tdzh:'',//土地证号
        ...
        //获取当前项目列表信息
        $scope.currentList = function () {
            $http.get("/departmentaudit/tobedelete?pageNo="+$scope.page.pageNo+"&pageSize="+$scope.page.pageSize,{params:$scope.searchItem}).then(function (result) {
        ...
        $scope.passSingle = function (item,flag) {
            var str = '确认不通过?';
            var url = "/departmentaudit/deletenotpassone";
            if(flag){
                str = '确认通过?';
                url = "/departmentaudit/deletepassone";
            }
            dialogWindow.confirm('提示',str,function () {
                $http.post(url,{
                    "id": item.objectId,
        ...
        $scope.passMultiple = function (flag) {
            var str = '批量确认不通过?';
            var url = "/departmentaudit/deletenotpassmore";
            if(flag){
                str = '批量确认通过?';
                url = "/departmentaudit/deletepassmore";
            }
            dialogWindow.confirm('提示',str,function () {
                $http.post(url,$scope.selectList).then(function (result) {
                    console.log(result.data);
